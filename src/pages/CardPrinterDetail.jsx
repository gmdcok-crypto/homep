...
import { products } from './CardPrinter';
import QuoteModal from '../components/QuoteModal';

const CardPrinterDetail = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        if (!product) {
            navigate('/products/card-printer');
            return;
        }
        window.scrollTo(0, 0);
            ...
            <div className="bg-gray-900 text-white relative overflow-hidden h-[400px] flex items-center">
                <div className="absolute inset-0 z-0">
                    <img
                        ...
                        alt="Card Printer Detail Background"
                        className="w-full h-full object-cover object-right opacity-50"
                    />
                </div>
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <Link to="/products/card-printer" className="inline-flex items-center px-4 py-2 bg-indigo-900/40 border border-indigo-400/20 text-white rounded-full hover:bg-indigo-900/60 transition-all mb-8 group">
...
export default CardPrinterDetail;
