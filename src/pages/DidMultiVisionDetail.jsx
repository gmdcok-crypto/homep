...
import { products } from './DidMultiVision';
import QuoteModal from '../components/QuoteModal';

const DidMultiVisionDetail = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        if (!product) {
            navigate('/products/did-multivision');
            return;
        }
        window.scrollTo(0, 0);
            ...
            <div className="bg-gray-900 text-white relative overflow-hidden h-[400px] flex items-center">
                <div className="absolute inset-0 z-0">
                    <img
                        ...
                        alt="DID Detail Background"
                        className="w-full h-full object-cover object-right opacity-50"
                    />
                </div>
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <Link to="/products/did-multivision" className="inline-flex items-center px-4 py-2 bg-indigo-900/40 border border-indigo-400/20 text-white rounded-full hover:bg-indigo-900/60 transition-all mb-8 group">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                    ...
                    <div className="bg-white rounded-3xl p-4 md:p-8 flex items-center justify-center border border-gray-100 shadow-sm relative group overflow-hidden h-[500px]">
                        <img
                            ...
                            alt={product.name}
                            className={`w-full h-full transition-transform duration-500 ${product.id === 'DID-MONITOR' ? 'object-cover scale-90' : 'object-contain mix-blend-multiply'}`}
                        />
...
export default DidMultiVisionDetail;
